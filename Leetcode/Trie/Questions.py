...
class WordDictionary:
    ...
    def search(self, word: str) -> bool:
        def dfs(i, node):
            ...
            if c == ".":
                for i in range(26):
                    ...
                    if nextNode is not None and dfs(i + 1, nextNode):
                        return True
                return False
            else:
        ...
        #                 queue.append((index+1, nextNode))

        #     else:
        #         if node.containsKey(c):
        #             queue.append((index + 1, node.get(c)))
        # return False
    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(i, j, word, node):
            if (
                not (0 <= i < m and 0 <= j < n)
                or (i, j) in visited
                or board[i][j] not in node.links
            ):
                return
            visited.add((i, j))
            node = node.links[board[i][j]]
            word += board[i][j]
            if node.isEnd:
                ans.add(word)
            dfs(i + 1, j, word, node)
